...
function segmentsFromText(text){
  const lines=(text||'').split(/\n/);const segs=[];let current={title:'Scene Elements',text:''};
...
function newCampaign(){if(!confirm("Start a new campaign and clear local state? Export first if you want a backup."))return;state=structuredClone(DEFAULT_STATE);try{localStorage.removeItem("sagaAtlasStoryDirectorV1");}catch(e){}saveState();render();try{window.dispatchEvent(new CustomEvent("saga-atlas-new-campaign"));}catch(e){} }function setStatus(msg){$("saveStatus").textContent=msg}
...
const ENTITY_TYPES = {
  ...
  encounter: { label: 'Encounters', singular: 'Encounter', icon: '☣' }
};
const ENTITY_SUBTYPES = {
  ...
  encounter: [
    ...
    {key:'lifeform', label:'Lifeform', icon:'☄'},
    {key:'hostile', label:'Hostile Contact', icon:'⚔'},
    {key:'hazard', label:'Hazard', icon:'⚠'},
    {key:'social', label:'Social Encounter', icon:'◉'},
    {key:'mystery', label:'Mystery Encounter', icon:'◇'}
  ]
};
const ENTITY_THUMBNAILS = Object.fromEntries(Object.entries(ENTITY_SUBTYPES).map(([type,rows])=>[type, rows.map(r=>r.icon)]));
const ENTITY_TAGS = {npc:[],location:[],faction:[],asset:[],encounter:[]};
...
function pruneUnusedEntityTags(){
  const catalog=entityTagCatalog();
  const used={}; Object.keys(ENTITY_TYPES||{}).forEach(type=>{ used[type]=new Set(); });
...
(function(){
  ...
  function installLoreEntityType(){
    try{
      ...
      if(typeof ENTITY_TYPES !== 'undefined' && !ENTITY_TYPES.encounter){
        ENTITY_TYPES.encounter = { label:'Encounters', singular:'Encounter', icon:'☣' };
      }
      if(typeof ENTITY_SUBTYPES !== 'undefined' && !ENTITY_SUBTYPES.encounter){
        ENTITY_SUBTYPES.encounter = [
          {key:'encounter', label:'Encounter', icon:'☣'},
          {key:'lifeform', label:'Lifeform', icon:'☄'},
          {key:'hostile', label:'Hostile Contact', icon:'⚔'},
          {key:'hazard', label:'Hazard', icon:'⚠'},
      ...
      if(typeof ENTITY_TAGS !== 'undefined' && !ENTITY_TAGS.encounter){
        ENTITY_TAGS.encounter = [];
      }
      if(typeof state !== 'undefined'){
        if(!state.entityTagCatalog || typeof state.entityTagCatalog !== 'object') state.entityTagCatalog = {};
        if(!Array.isArray(state.entityTagCatalog.lore)) state.entityTagCatalog.lore = [];
        if(!Array.isArray(state.entityTagCatalog.encounter)) state.entityTagCatalog.encounter = [];
      }
  ...
  function normalizeOneEntity(ent, i){
    ...
    if(ent.type === 'encounters' || ent.type === 'creature' || ent.type === 'creatures' || ent.type === 'lifeform') ent.type = 'encounter';
